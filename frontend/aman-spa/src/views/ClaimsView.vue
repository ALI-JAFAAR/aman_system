...
async function load() {
  ...
  error.value = null
  try {
    const { data } = await api.get('/v1/claims', { params: { per_page: 20 } })
    claims.value = data?.data?.data ?? []
  } catch (e: any) {
    error.value = e?.response?.data?.message ?? 'فشل تحميل المطالبات'
  } finally {
    loading.value = false
  }
}
...
<template>
  <div>
    <div class="flex items-center justify-between mb-4">
      <h1 class="text-xl font-semibold text-slate-900">المطالبات</h1>
    ...
    <div v-else class="overflow-x-auto border border-slate-200 rounded-xl">
      <table class="min-w-full text-sm">
        ...
        <tbody>
          <tr v-for="c in claims" :key="c.id" class="border-t border-slate-200">
            <td class="p-3">{{ c.id }}</td>
            <td class="p-3">{{ c.type }}</td>
            <td class="p-3">{{ c.status }}</td>
            <td class="p-3">{{ c.amount_requested }}</td>
          </tr>
