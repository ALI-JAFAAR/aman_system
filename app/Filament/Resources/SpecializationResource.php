    ...
    public static function table(Table $table): Table
    {
        return $table
            ->columns([
                ...
                Tables\Columns\TextColumn::make('profession.name')->label('المهنة')->sortable()->toggleable(),
                Tables\Columns\TextColumn::make('code')->label('الكود'),
                Tables\Columns\IconColumn::make('is_active')->boolean()->label('فعّال'),
            ])
            ->filters([
                Tables\Filters\SelectFilter::make('profession')->label('المهنة')->relationship('profession', 'name'),
            ])
